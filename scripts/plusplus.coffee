...
module.exports = (robot) ->
    ...
    if score?
      message = if reason?
                  if reasonScore == 1 or reasonScore == -1
                    "#{name}'s light level is #{score}, #{reasonScore} of which is for #{reason}."
                  else
                    "#{name}'s light level is #{score}, #{reasonScore} of which are for #{reason}."
                else
                  if score == 1
                    "#{name}'s light level is #{score}"
                  else
                    "#{name}'s light level is #{score}"
  ...
  robot.respond /score (for\s)?(.*)/i, (msg) ->
    ...
    reasonString = if typeof reasons == 'object' && Object.keys(reasons).length > 0
                     "#{name}'s light level is #{score}. here are some raisins:" +
